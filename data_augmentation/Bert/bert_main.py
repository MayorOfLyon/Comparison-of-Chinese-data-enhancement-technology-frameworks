...
class BertAugmentor(object):
    ...
    def word_replace(self,query):
        ...
        for index_, word_len in index_map.items():
            ...
            for i in range(word_len):
                if index_ + i >= len(word_ids_):
                    print(f"发生超界！index_: {index_}, i: {i}, len(word_ids_): {len(word_ids_)}, word_len: {word_len}")
    ...
    #插入
    def insert_word2queries(self, query:str):
        output = self.word_insert(query)
        return output
    #替换
    def replace_word2queries(self, query:str):
        output = self.word_replace(query)
        return output
